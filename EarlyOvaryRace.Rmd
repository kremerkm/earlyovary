...
HGS.HB <- filter(HGS.ES, HGS.ES$Race == "Black" | HGS.ES$Race == "Hisp")
...
```{r, echo = FALSE}
HGS.HB$Race <- factor(HGS.HB$Race, levels = c('Hisp', 'Black'))
fit.byHB <- survfit(Surv(SurvMonths, COD) ~ Race, HGS.HB)
ggsurvplot(fit.byHB, HGS.HB, pval = TRUE, xlab = "Months", break.time.by = 12, break.y.by = 0.1, legend.title = "Race", title = "Survival of early stage HGSOC patients stratified by Race", legend.labs = c("Any Hispanic", "Black"), legend = "bottom", palette = "Set1", ggtheme = theme_gray())
...
```{r, echo = FALSE}
HGS.Hisp.N0 <- filter(HGS.ES, HGS.ES$Race == "Hisp" & HGS.ES$Nodes_Pos == "No")
...
```{r, echo = FALSE}
HGS.Hisp.Nx <- filter(HGS.ES, HGS.ES$Race == "Hisp" & HGS.ES$Nodes_Pos == "Unk")
...
```{r, echo = FALSE}
HGS.ES.forest <- rename(HGS.ES,
+ Stage = TNM.Stage,
+ Laterality = Lat,
+ Chemotherapy = Chemo,
+ `Race Group` = Race.Group,
+ Lymphadenectomy = Nodes_Ex)
HGS.ES.forest$`Race Group` <- factor(HGS.ES.forest$`Race Group`, levels = c('White', 'Hisp', 'Black', 'Other'))
ph.ES.forest <- coxph(Surv(SurvMonths, COD) ~ Stage + Laterality + Chemotherapy + `Race Group` + Lymphadenectomy, data = HGS.ES.forest)
summary(ph.ES.forest)
```