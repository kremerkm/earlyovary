...
KMbyLND <- ggsurvplot(fit.byLND, HGS.ES, xlab = "Months", break.time.by = 12, break.y.by = 0.1, title = "Survival of presumed early-stage HGSOC stratified \nby adequacy of lymph node dissection", palette = "Set1", ggtheme = theme_gray(), legend = "bottom", legend.title = "Lymphadenectomy", legend.labs = c("Adequate", "Inadequate", "None"), pval = TRUE)
ggsave("KMbyLND.png", print(KMbyLND), height = 4, width = 5, units = c("in"))
...
```{r, echo=FALSE}
fit.byLND.chemo <- survfit(Surv(SurvMonths, COD) ~ LND + Chemo, data = HGS.ES)
ggsurvplot(fit.byLND.chemo, HGS.ES, xlab = "Months", break.time.by = 12, break.y.by = 0.1, title = "Survival of presumed early-stage HGSOC stratified \nby lymph node dissection and receipt of chemotherapy", legend = "bottom", legend.title = "Lymphadenectomy", legend.labs = c("Adequate, no Chemo", "Adequate with Chemo", "Inadequate, no Chemo", "Inadequate with Chemo", "None, no Chemo", "None with Chemo"), palette = "Set1", ggtheme = theme_gray())
pairwise_survdiff(Surv(SurvMonths, COD) ~ LND + Chemo, HGS.ES)

ph.byLND.chemo <- coxph(Surv(SurvMonths, COD) ~ LND + Chemo, HGS.ES)
...
KM.ES.chemo <- ggsurvplot(fit.ES.chemo, HGS.ES.chemo, pval = TRUE, xlab = "Months", break.time.by = 12, break.y.by = 0.1, legend.title = "Stage", legend = "bottom", legend.labs = c("T1N0M0", "T1NxMO"), title = "Survival of presumed early-stage HGSOC \nthat received chemotherapy", palette = "Set1", ggtheme = theme_gray())
