...
HGS = mutate(HGS, Race.Group = ifelse(HGS$Race == "Black", "Black",
                                      ifelse(HGS$Race == "White", "White",
                                             ifelse(HGS$Race == "Hisp", "Hisp", "Other"))))
...
## Facet by chemotherapy

```{r, echo=FALSE}
fit.byLND <- survfit(Surv(SurvMonths, COD) ~ Nodes_Ex, data = HGS.ES)
ggsurvplot_facet(fit.byLND, HGS.ES, facet.by = "Chemo", xlab = "Months", break.time.by = 12, break.y.by = 0.1, title = "Survival of presumed early-stage HGSOC stratified \nby lymph node dissection", legend = "bottom", legend.title = "Lymphadenectomy", legend.labs = c("Adequate", "Inadequate", "None"), palette = "Set1", ggtheme = theme_gray())
pairwise_survdiff(Surv(SurvMonths, COD) ~ Nodes_Ex + Chemo, HGS.ES)
```
```{r, echo = FALSE}
ph.byLND.chemo <- coxph(Surv(SurvMonths, COD) ~ Nodes_Ex + Chemo, HGS.ES)
ggforest(ph.byLND.chemo, HGS.ES)
